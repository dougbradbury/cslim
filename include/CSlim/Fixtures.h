...
#define SLIM_FIXTURES                           \
  void AddFixtures(StatementExecutor* executor) \
  {

#define SLIM_END }

#define PRIVATE_REGISTER_FIXTURE(name) \
  StatementExecutor_RegisterFixture(executor, #name, name##_Create, name##_Destroy);

#define SLIM_CREATE_EMPTY_FIXTURE(name) \
  void name##_Register(StatementExecutor* executor) { PRIVATE_REGISTER_FIXTURE(name) }

#define SLIM_CREATE_FIXTURE(name)                                 \
  static const char* fixtureName = #name;                         \
...
#define SLIM_CONSTRUCTOR_ERROR(errorHandler, reason) \
  StatementExecutor_ConstructorError(errorHandler, reason);
